...
$product_query = mysqli_query($conn, "SELECT * FROM products WHERE name LIKE '%Mustard%' OR name LIKE '%Aba%' LIMIT 1");
$product = mysqli_fetch_assoc($product_query);

if (!$product) {
    die("Black Mustard Seeds product not found in database! Please check products table.");
}

// Use actual data from database
$spice_id = $product['product_id'];
$spice_name = $product['name'];
$spice_desc = $product['description']; // Description from database
$product_price = $product['price'];
$product_stock = $product['stock'];

// USE VIEW PAGE SPECIFIC IMAGE
$spice_image = '../../assets/images/Black-Mustard-Seeds-powder.jpg'; // Your detailed view image
...
$spice_sinhala = 'Aba Kudu - අබ ‌කුඩු';
